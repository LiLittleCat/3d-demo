...
function SegmentComponent({ segment }: { segment: Segment }) {
  ...
  return (
    <Line points={[start, end]} color={color} lineWidth={1} dashed={false} />
  );
}
...
function SurfaceComponent({
  ...
  surface: Surface | SurfaceWithPoints;
}) {
  let point: THREE.Vector3;
  let normal: THREE.Vector3;
  let color =
    surface.color || Math.floor(Math.random() * 16777215).toString(16);
  let width = surface.width || 10;
  let height = surface.height || 10;
  let normalLength = surface.normalLength || 10;
